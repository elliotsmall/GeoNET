...
func (store *Store) LookupAgent(ctx context.Context, agentID uuid.UUID) (string, error) {
	var token string
	err := store.pool.QueryRow(ctx, "SELECT token FROM agents WHERE id = $1", agentID).Scan(&token)
	if err != nil {
...
func (store *Store) InsertAgent(ctx context.Context, agentID uuid.UUID, tokenHash string, issuedAt time.Time) error {
	_, err := store.pool.Exec(ctx, "INSERT INTO agents (id, token, issued_at) VALUES ($1, $2, $3)", agentID, tokenHash, issuedAt)
	if err != nil {
